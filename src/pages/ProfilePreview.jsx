...
import { getCurrentUser, getMyStory } from '../utils/api.js';
...
const ProfilePreview = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError('');
      try {
        const [userData, storyData] = await Promise.all([
          getCurrentUser(),
          getMyStory(),
        ]);
    ...
    <div className="min-h-screen bg-premium-mesh pb-[calc(100px+env(safe-area-inset-bottom,0px))]">
      <div className="mx-auto max-w-2xl px-4 py-6">
        ...
        <motion.div
          ...
          className="mt-4 rounded-[28px] border border-white/60 bg-white/95 shadow-xl overflow-hidden"
        >
          <div className="relative h-72">
            ...
            <div className="absolute bottom-4 left-4 right-4 text-white">
              ...
              <p className="text-sm text-white/90">
                {user.age} • {user.gender} • {user.location_city}
              </p>
            </div>
          </div>
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-slate-600">
              <span className="rounded-full bg-slate-50 px-3 py-1">👶 {user.has_kids ? `${user.num_kids} kids` : 'No kids'}</span>
              <span className="rounded-full bg-slate-50 px-3 py-1">{user.smoker ? '🚬 Smoker' : '🚭 Non-smoker'}</span>
              <span className="rounded-full bg-slate-50 px-3 py-1">{user.drinks_alcohol ? '🍷 Drinks' : '🚫 No alcohol'}</span>
              <span className="rounded-full bg-slate-50 px-3 py-1">{user.nationality || 'Nationality'}</span>
            </div>
