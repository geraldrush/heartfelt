...
async function generateToken(userId, secret) {
  ...
  const secretKey = encoder3.encode(secret);
  return await new SignJWT({ sub: userId }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("30d").sign(secretKey);
}
...
__name(getUserByGoogleId, "getUserByGoogleId");
...
async function getTokenTransactions(db, userId, limit, offset) {
...
__name(getTokenTransactions, "getTokenTransactions");
async function createTokenRequest(db, requestData) {
...
async function getPendingTokenRequests(db, userId) {
  const { results } = await db.prepare(
    `SELECT
        tr.id,
        tr.requester_id,
...
__name(getPendingTokenRequests, "getPendingTokenRequests");
async function updateTokenRequestStatus(db, requestId, status) {
  await db.prepare(
...
__name(updateTokenRequestStatus, "updateTokenRequestStatus");
function buildDistanceQuery(userLat, userLng) {
  return `(6371 * acos(
...
__name(buildDistanceQuery, "buildDistanceQuery");
async function getConnectionRequestById(db, requestId) {
  return db.prepare("SELECT * FROM connection_requests WHERE id = ?").bind(requestId).first();
}
__name(getConnectionRequestById, "getConnectionRequestById");
async function getSentConnectionRequests(db, userId) {
  const { results } = await db.prepare(
    `SELECT
        cr.id,
        cr.receiver_id,
...
__name(getSentConnectionRequests, "getSentConnectionRequests");
async function getReceivedConnectionRequests(db, userId) {
  const { results } = await db.prepare(
    `SELECT
        cr.id,
        cr.sender_id,
...
__name(getReceivedConnectionRequests, "getReceivedConnectionRequests");
async function getConnections(db, userId) {
  const { results } = await db.prepare(
    `SELECT
        c.id,
        c.user_id_1,
...
async function updateConnectionRequestStatus(db, requestId, status, respondedAt) {
  await db.prepare(
    `UPDATE connection_requests
       SET status = ?, responded_at = ?, updated_at = CURRENT_TIMESTAMP
       WHERE id = ?`
  ).bind(status, respondedAt || (/* @__PURE__ */ new Date()).toISOString(), requestId).run();
}
__name(updateConnectionRequestStatus, "updateConnectionRequestStatus");
async function checkExistingConnection(db, userId1, userId2) {
  const row = await db.prepare(
    `SELECT 1 FROM connections
       ...
       LIMIT 1`
  ).bind(userId1, userId2, userId2, userId1, userId1, userId2, userId2, userId1).first();
  return Boolean(row);
}
__name(checkExistingConnection, "checkExistingConnection");
async function expireOldRequests(db) {
  await db.prepare(
    `UPDATE connection_requests
...
__name(expireOldRequests, "expireOldRequests");
...
async function getMessagesByConnection(db, connectionId, limit, offset, beforeTimestamp) {
...
__name(getMessagesByConnection, "getMessagesByConnection");
async function updateMessageStatus(db, messageId, status) {
  await db.prepare("UPDATE messages SET status = ? WHERE id = ?").bind(status, messageId).run();
}
__name(updateMessageStatus, "updateMessageStatus");
async function updateMultipleMessageStatus(db, messageIds, status) {
  ...
  ).bind(status, ...messageIds).run();
}
__name(updateMultipleMessageStatus, "updateMultipleMessageStatus");
async function getUnreadCounts(db, userId) {
  const { results } = await db.prepare(
...
async function verifyUserInConnection(db, connectionId, userId) {
  const row = await db.prepare(
    `SELECT 1 FROM connections
...
__name(verifyUserInConnection, "verifyUserInConnection");
async function getMessageIdsForUser(db, userId, messageIds) {
  if (!Array.isArray(messageIds) || messageIds.length === 0) {
    return [];
  }
  const placeholders = messageIds.map(() => "?").join(", ");
  ...
  ).bind(...messageIds, userId, userId).all();
  return results.map((row) => row.id);
}
__name(getMessageIdsForUser, "getMessageIdsForUser");
async function getReferenceData(db) {
  const religions = await db.prepare("SELECT id, name FROM religions WHERE is_active = 1").all();
  const races = await db.prepare("SELECT id, name FROM races WHERE is_active = 1").all();
  const education_levels = await db.prepare("SELECT id, name FROM education_levels WHERE is_active = 1").all();
...
__name(getReferenceData, "getReferenceData");
...
var emailSignupSchema = external_exports.object({
  email: external_exports.string().email({ message: "Enter a valid email address." }),
  password: external_exports.string().min(8, { message: "Password must be at least 8 characters." }).regex(passwordRegex, {
    message: "Password must include at least one uppercase letter and one number."
  }),
  full_name: external_exports.string().min(2, { message: "Full name is required." }),
  age: external_exports.coerce.number().int({ message: "Age must be a whole number." }).min(18, { message: "You must be at least 18 years old." }),
  gender: external_exports.enum(["male", "female", "non-binary", "other"], {
    message: "Gender is required."
  }),
  nationality: external_exports.string().min(1, { message: "Nationality is required." }),
  location_city: external_exports.string().min(1, { message: "City is required." }),
  location_province: external_exports.string().min(1, { message: "Province is required." })
});
...
var googleAuthSchema = external_exports.object({
  credential: external_exports.string().min(1, { message: "Google credential is required." })
});
var updateProfileSchema = external_exports.object({
  age: external_exports.number().int().min(18),
  gender: external_exports.enum(["male", "female", "non-binary", "other"]),
  nationality: external_exports.string().min(1),
  religion: external_exports.string().min(1),
  race: external_exports.string().min(1),
  education: external_exports.string().min(1),
  has_kids: external_exports.boolean(),
  num_kids: external_exports.number().int().refine((value) => [0, 1, 2, 3].includes(value), {
    message: "Number of kids must be 0, 1, 2, or 3+."
  }),
  smoker: external_exports.boolean(),
  drinks_alcohol: external_exports.boolean(),
  location_city: external_exports.string().min(1),
  location_province: external_exports.string().min(1)
});
var createStorySchema = external_exports.object({
  story_text: external_exports.string().min(50, { message: "Story must be at least 50 characters." }),
  image_ids: external_exports.array(external_exports.string()).min(1).max(5)
});
var tokenTransferSchema = external_exports.object({
  recipient_id: external_exports.string().uuid(),
  amount: external_exports.number().int().min(1).max(1e3),
  message: external_exports.string().max(200).optional()
});
var tokenHistorySchema = external_exports.object({
...
var connectionRequestSchema = external_exports.object({
  receiver_id: external_exports.string().uuid(),
  message: external_exports.string().max(200).optional()
});
var connectionActionSchema = external_exports.object({
  request_id: external_exports.string().uuid()
});
var paymentInitiateSchema = external_exports.object({
  package_id: external_exports.string().uuid()
});

// src/routes/auth.js
var auth = new Hono2();
var getSignupBonusTokens = /* @__PURE__ */ __name((c) => {
  const raw2 = Number(c.env.SIGNUP_BONUS_TOKENS);
  return Number.isFinite(raw2) && raw2 > 0 ? raw2 : 50;
...
__name(userResponse, "userResponse");
...
auth.post("/email-login", async (c) => {
  ...
  const isValid2 = await verifyPassword(parsed.data.password, user.password_hash);
  if (!isValid2) {
    return c.json({ error: "Invalid email or password." }, 401);
  }
...
stories.post("/upload-image", authMiddleware, async (c) => {
  ...
  if (!originalFile || !blurredFile || typeof originalFile === "string" || typeof blurredFile === "string") {
    return c.json({ error: "Original and blurred image files are required." }, 400);
  }
  const allowedTypes = /* @__PURE__ */ new Set(["image/jpeg", "image/png", "image/webp"]);
...
stories.post("/create-story", authMiddleware, async (c) => {
  ...
  await db.prepare("INSERT INTO stories (id, user_id, story_text, is_active) VALUES (?, ?, ?, 1)").bind(storyId, userId, parsed.data.story_text).run();
  for (const imageId of parsed.data.image_ids) {
    ...
    ).bind(generateId(), storyId, originalUrl, blurredUrl, "completed").run();
  }
...
stories.get("/feed", authMiddleware, async (c) => {
  ...
  const maxDistance = Number.isFinite(Number(maxDistanceRaw)) ? Number(maxDistanceRaw) : 100;
  const parseBool = /* @__PURE__ */ __name((value) => {
  ...
  const distanceFormula = hasUserLocation ? buildDistanceQuery(userLat, userLng) : "999999";
  const distanceSelect = hasUserLocation ? `CASE WHEN users.location_lat IS NULL OR users.location_lng IS NULL THEN 999999 ELSE ${distanceFormula} END` : "999999";
  const conditions = [
    "stories.is_active = 1",
  ...
  const countQuery = `
    ...
    ${whereClause}
  `;
  const countRow = await db.prepare(countQuery).bind(...joinParams, ...params).first();
  ...
  const feedQuery = `
    ...
    LIMIT ? OFFSET ?
  `;
  const { results } = await db.prepare(feedQuery).bind(...joinParams, ...params, limit, offset).all();
  const origin = new URL(c.req.url).origin;
  const stories2 = results.map((row) => {
    const hasImage = Boolean(row.blurred_image_url);
    const imageUrl = hasImage ? `${origin}/api/stories/${row.story_id}/blurred` : null;
    return {
      ...row,
  ...
  return c.json({
    stories: stories2,
    total,
    has_more: offset + stories2.length < total
  });
});
stories.get("/:storyId/images", async (c) => {
  ...
  ).bind(storyId, "completed").all();
  const origin = new URL(c.req.url).origin;
  const images2 = results.map((row) => ({
    id: row.id,
    blurred_url: `${origin}/api/stories/${storyId}/blurred`,
    processing_status: row.processing_status,
    created_at: row.created_at
  }));
  return c.json({ images: images2 });
});
...
connections.post("/request", authMiddleware, async (c) => {
  ...
  const message2 = parsed.data.message || "Connection request sent";
...
connections.get("/counts", authMiddleware, async (c) => {
  ...
  const totalConnections = await db.prepare(
...
images.post("/process", authMiddleware, async (c) => {
  ...
  try {
    ...
    if (!object) {
      ...
      ).bind("failed", storyId, originalKey).run();
      return c.json({ error: "Image not found." }, 404);
    }
    if (!c.env.AI) {
    ...
    const results = aiResult?.faces || aiResult?.result?.faces || aiResult?.results || aiResult?.items || [];
    const faces = Array.isArray(results) ? results.filter((item) => {
    ...
    ).bind("completed", JSON.stringify(faces), faces.length, storyId, originalKey).run();
    return c.json({ faces });
  } catch (error) {
    await db.prepare(
      "UPDATE story_images SET processing_status = ? WHERE story_id = ? AND original_url = ?"
...
// src/utils/payfast.js
import { createHash, timingSafeEqual } from "node:crypto";
var encodeValue = /* @__PURE__ */ __name((value) => encodeURIComponent(String(value)).replace(/%20/g, "+"), "encodeValue");
function generateSignature(data, passphrase) {
  const payload = Object.keys(data).sort().filter((key) => data[key] !== void 0 && data[key] !== null && data[key] !== "").map((key) => `${key}=${encodeValue(data[key])}`).join("&");
  const base = passphrase ? `${payload}&passphrase=${encodeValue(passphrase)}` : payload;
  return createHash("md5").update(base).digest("hex");
}
__name(generateSignature, "generateSignature");
function verifySignature(data, receivedSignature, passphrase) {
...
__name(getPayfastUrl, "getPayfastUrl");
function validateItnSource(_request) {
  return true;
}
__name(validateItnSource, "validateItnSource");
...
payments.post("/initiate", authMiddleware, async (c) => {
  ...
  ).bind(parsed.data.package_id).first();
  if (!packageRow) {
  ...
  await db.prepare(
    `INSERT INTO payments (id, user_id, package_id, amount_cents, status)
       VALUES (?, ?, ?, ?, 'pending')`
  ).bind(paymentId, userId, packageRow.id, packageRow.price_cents).run();
...
payments.post("/notify", async (c) => {
  ...
  const grossCents = Number.isFinite(grossAmount) ? Math.round(grossAmount * 100) : NaN;
  ...
  await db.prepare(
    `UPDATE payments
       SET status = 'failed',
           payfast_payment_id = ?
       WHERE id = ? AND status = 'pending'`
  ).bind(payload.pf_payment_id || null, paymentId).run();
  return c.text("OK", 200);
});
...
var ChatRoom = class {
  ...
  broadcast(message2, excludeUserId) {
...
// src/index.js
var app = new Hono2();
var defaultOrigins = ["http://localhost:5173", "https://heartfelt.pages.dev"];
app.use(
  "/*",
  cors({
    origin: /* @__PURE__ */ __name((origin, c) => {
      const raw2 = c.env.CORS_ORIGIN || "";
      const allowed = raw2.split(",").map((value) => value.trim()).filter(Boolean);
...
app.route("/api/images", images_default);
